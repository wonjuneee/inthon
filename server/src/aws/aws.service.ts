...
export class AwsService {
  ...
  constructor(private config: ConfigType<typeof awsConfig>) {
    // AWS S3 클라이언트 초기화. 환경 설정 정보를 사용하여 AWS 리전, Access Key, Secret Key를 설정.
    this.s3Client = new S3Client({
      region: config.region, // AWS Region
      credentials: {
        accessKeyId: config.s3AccessKey, // Access Key
        secretAccessKey: config.s3SecretAccessKey, // Secret Key
      },
    });
  }
  ...
  async imageUploadToS3(
    ...
    const command = new PutObjectCommand({
      Bucket: this.config.s3BucketName, // S3 버킷 이름
    ...
    const bucket = this.config.s3BucketName;
